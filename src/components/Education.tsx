...
function EduCard({ deg }: { deg: (typeof education)[number] }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className="spotlight-card rounded-xl p-6 border border-color"
  ...
  return (
    <section id="education" className="py-20 px-6 max-w-5xl mx-auto">
      <motion.div variants={reduced ? fadeUp : staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }}>
        <SectionHeading eyebrow="Education" title="Where I studied." className="mb-10" />
        <div className="grid md:grid-cols-2 gap-6">
          {education.map(deg => (
            <EduCard key={deg.institution} deg={deg} />
          ))}
        </div>
      </motion.div>
    </section>
